...
public:
    Field();
    Field(std::vector<Point> coordinates); //устанавливаем начальное состояние клеток
    int getSize() const;
...
public:
    ...
    int getGlobIteration () const; //возвращаем номер итерации
    void computeIteration(); //расчитываем следующее состояние поля
    const std::string& getName() const; //возвращаем название игры
    const Field& getField() const; //возвращаем поле
};
